...
def capture_handler():
    ...
    try:
        ...
        if register_mode:
            ...
            if user_id is None:
                return
            if str(user_id) not in user_templates:
                user_id_str = str(current_fid)  # Generate a new ObjectId if needed
                user_templates[str(user_id)] = []

            # if user_id_str not in user_templates:
                templates = user_templates[str(user_id)]
            ...
            if not templates or zkfp2.DBMatch(bytes.fromhex(templates[-1]), tmp_bytes) > 0:
                ...
                socketio.emit("fingerprint_scan", {"user_id": user_id, "step": len(templates)}, namespace='/')
                ...
                if len(templates) == 3:
                    ...
                    # user_id_int = int(user_id[:8], 16)  # Convert first 8 hex chars to int

                    logger.info(f"✅ User {user_id_str} enrolled {reg_temp} successfully. {user_id} {templates}")
                    # Add to local fingerprint database
                    zkfp2.DBAdd(user_id, reg_temp)
                    logger.info(f"✅ User {user_id} enrolled successfully. {user_id_int}")
